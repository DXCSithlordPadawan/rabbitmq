...
## 6. TLS Generation
 - Ensure you have the certificates prepared and copied to:
 - using command:
```
./request-cert.sh -n "rabbitmq.aip.dxc.com" -s "rabbitmq.aip.dxc.com,rabbitmq,rabbitmq.aip,aip-rabbitmq.taild43680.ts.net,aip-rabbitmq" -i "192.168.0.130,100.85.64.121"
```
 - Step 1: Locate and Examine Your Generated Certificates
 -         First, let's find the certificates generated by your script:
```
# Navigate to where your certificates were generated
cd ~/certificates  # or wherever your request-cert.sh output directory is

# List the generated certificate files
ls -la *rabbitmq*

# You should see files like:
# bundle_[timestamp].pem
# certificate_[timestamp].pem  
# private_key_[timestamp].pem
# ca_certificate.pem
```
 - Step 2: Copy and Rename Certificates for RabbitMQ
          Copy your generated certificates to the RabbitMQ certificates directory with the expected names:
```
# Create RabbitMQ certificate directory
sudo mkdir -p /etc/rabbitmq/certs

# Copy and rename your certificates (replace [timestamp] with actual timestamp)
sudo cp certificate_[timestamp].pem /etc/rabbitmq/certs/rabbitcert.pem
sudo cp private_key_[timestamp].pem /etc/rabbitmq/certs/rabbitkey.pem  
sudo cp ca_certificate.pem /etc/rabbitmq/certs/ca_root.pem
sudo cp bundle_[timestamp].pem /etc/rabbitmq/certs/rabbitbundle.pem
